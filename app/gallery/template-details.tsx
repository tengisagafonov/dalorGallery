...
export function TemplateDetails({ controller, detailsRef, locale, t }: Props) {
  ...
  return (
        <aside ref={detailsRef} className="scroll-mt-3 border-t border-line bg-surface-sunken p-5 lg:border-l lg:border-t-0">
          <div className="rounded-[22px] border border-line bg-surface-raised p-5 shadow-[0_4px_20px_rgba(70,55,40,0.035)]">
            <div className="flex gap-4 border-b border-line pb-5">
              <button
                type="button"
                onClick={() => selected.cover && setIsCoverPreviewOpen(true)}
                className={`relative h-32 w-24 shrink-0 overflow-hidden rounded-lg bg-gradient-to-br p-3 text-left transition hover:scale-[1.02] focus:outline-none focus:ring-2 focus:ring-accent focus:ring-offset-2 focus:ring-offset-surface-raised ${selected.style} ${selected.cover ? "cursor-zoom-in" : "cursor-default"}`}
              ...
              <div className="min-w-0 pt-1">
                ...
                <span className="mt-2 inline-block rounded-full border border-line bg-subtle px-2.5 py-1 text-[10px] font-medium uppercase tracking-wide text-ink-muted">
                ...
                <button
                  ...
                  className="mt-3 inline-flex items-center gap-2 rounded-full border border-line-strong bg-subtle px-3 py-2 text-[11px] font-semibold text-ink shadow-sm transition hover:-translate-y-px hover:bg-elevated hover:shadow-md focus:outline-none focus:ring-2 focus:ring-line-strong focus:ring-offset-2 focus:ring-offset-surface-raised"
                >
                  <span className="flex items-center gap-2">
                    <span className="flex size-6 items-center justify-center rounded-full bg-elevated">
                      <Icon name="eye" className="size-4" />
                    </span>
                    {t.previewPrompt}
                  </span>
                  <Icon name="arrow" className="size-3.5 text-ink-faint" />
                </button>
              </div>
            </div>

            <div className="pt-5">
              <p className="mb-2 font-mono text-[9px] uppercase tracking-[0.2em] text-ink-faint">02 — Customize</p>
              <h3 className="text-[18px] font-black italic tracking-tight">{t.customize}.</h3>
              <p className="mt-1 text-[12px] leading-5 text-ink-muted">{t.customizeHint}</p>
                ...
                {selected.fields.map((field) => {
                  ...
                  <label key={`${selected.id}-${field.key}`} className="block">
                    <span className="mb-1.5 block text-[11px] font-semibold">
                      {label} {field.optional && <span className="font-normal text-ink-faint">(optional)</span>}
                    </span>
                    ...
                    {field.type === "textarea" ? (
                      <textarea
                        ...
                        rows={4}
                        className="w-full resize-y rounded-xl border border-line bg-elevated px-3 py-2.5 text-[12px] leading-5 outline-none placeholder:text-ink-faint focus:border-line-hover focus:ring-2 focus:ring-ring"
                      />
                    ) : <div className="flex items-center gap-2 rounded-xl border border-line bg-elevated px-3 py-2.5 focus-within:border-line-hover focus-within:ring-2 focus-within:ring-ring">
                      {field.type === "color" && (
                        <input
                          type="color"
                          value={
              ...
              <p className="mt-6 text-center text-[10px] text-ink-faint">{t.privacy}</p>
            </div>
          </div>
        </aside>
  );
