...
Neue Vorlagen für die Kategorie **"E-Commerce"** — exakt so geschrieben, mit Bindestrich.

Vorlagen kommen aus meiner Pinterest-Pinnwand **"E-Commerce"**:
https://at.pinterest.com/sukhjitmultani/e-commerce/

Dort liegen **51 Pins**. Jeder Pin ist die Vorlage für genau eine Galerie-Vorlage.

Die Pins decken ab: Audio (Kopfhörer, Earbuds), Gaming (Konsolen, Controller, Mäuse),
Beauty und Skincare (Seren, Cremes, Masken), Mode und Schuhe, dazu Notebook, Protein
und Snacks. Achte darauf, dass die Vorlagen diese Bandbreite abbilden und nicht zur
Hälfte Kopfhörer werden.

## Schritt 0: Bestand prüfen, NICHT löschen

In "E-Commerce" liegen bereits 156 Dokumente, davon 53 veröffentlicht. Sieh sie dir an
und sag mir, ob sie brauchbar sind oder ersetzt werden sollen. Gelöscht wird nichts,
bevor ich das nach deinem Bericht ausdrücklich freigebe.

Falls ich das Löschen freigebe: **vorher Sicherungskopie anlegen** —
`backend/.tmp/data.db` nach `backend/.tmp/data.db.bak-vor-ecom-loeschung` kopieren.
Das ist Pflicht, das Löschen ist sonst nicht rückgängig zu machen.

## Ablauf pro Vorlage

1. Pin ansehen. Bildkonzept ableiten: Produkttyp, Hintergrund, Blickwinkel,
   Lichtstimmung, Textaufbau. **Nicht 1:1 nachbauen** — in die Richtung, mit eigener
   erfundener Marke.
2. Bild mit Meta AI erzeugen (www.meta.ai, ist eingeloggt).
3. Herunterladen, nach `C:\Users\User\Downloads\E-Commerc` verschieben.
...
- Vorlagen, die du **schon veröffentlicht hast** (aktuell 34 in "E-Commerce"): auf Entwurf
...
- Bilder sofort von `C:\Users\User\Downloads` nach `C:\Users\User\Downloads\E-Commerc`
  verschieben. Der Ordner heißt genau so, ohne "e" am Ende. Chrome lädt immer nach
  Downloads, ein anderes Ziel lässt sich nicht einstellen.
...
Jede Vorlage bekommt einen **eigenen** Prompt, abgeleitet aus ihrem Pin. Da die Pins
verschiedene Produkte und Szenen zeigen, ergibt sich das von selbst — aber prüf es am
Ende ausdrücklich nach.

**6 bis 8 Variablen** pro Vorlage. Sinnvoll heißt: ändert sichtbar etwas im Bild. Zum
Beispiel Produkttyp, Produktfarbe, Material, Hintergrund, Lichtstimmung, Markenname,
Headline, Rabattangabe, Call-to-Action.
...
voll mit Apple, Sony, JBL, Nike, BOSE). Von solchen Pins nur Bildaufbau, Licht und
Typografie übernehmen. Der Markenname ist immer die Variable `brand_name`.
...
1. Bild nach `backend/public/__tmp_ecom.webp` kopieren (Strapi serviert `public/` unter
...
2. Im Admin-Tab per `javascript_tool` das JWT holen. **Beide Quellen prüfen:** früher lag
   ...
   (`JSON.parse(...)` nötig). Dann `fetch('/__tmp_ecom.webp')` → Blob → FormData →
...
Sobald `C:\Users\User\Downloads\E-Commerc` **50 Bilder** enthält: keine weiteren Bilder
...
```json
[
  { "file": "01-novarc-headphones.webp", "verdict": "approved" },
  { "file": "02-veridan-face-wash.webp", "verdict": "rejected",
...
- Es laufen mehrere Claude-Instanzen parallel an anderen Kategorien. Halte dich an den
  Temp-Dateinamen `__tmp_ecom.webp`, sonst überschreibt ihr euch gegenseitig.
