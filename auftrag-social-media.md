...
Neue Vorlagen für die Kategorie **"Social Media"** — exakt so geschrieben.

Vorlagen kommen aus meiner Pinterest-Pinnwand **"Social Media"**:
https://at.pinterest.com/sukhjitmultani/social-media/

Dort liegen **53 Pins**. Jeder Pin ist die Vorlage für genau eine Galerie-Vorlage.

Die Pins decken ab: Mode und Streetwear (Hoodies, Oversized-Shirts, Sneaker, Denim,
Lederjacken), Beauty und Skincare (Face Wash, Seren, Body Lotion, Masken), Audio
(Kopfhörer, Earbuds) und einzelne Nahrungsergänzung. Dazu Sale- und
Neuheiten-Ankündigungen mit großer Typografie.

## Schritt 0: Bestand prüfen

In "Social Media" liegt aktuell nur **1 veröffentlichte** Vorlage — die Kategorie ist
praktisch leer und steht auf Platz 5 der Sidebar. Hier ist also nichts zu löschen,
es wird nur aufgebaut. Sieh dir die eine bestehende Vorlage an, damit du weißt, wie
eine fertige Vorlage in diesem Projekt aussieht.

## Ablauf pro Vorlage

1. Pin ansehen. Bildkonzept ableiten: Produkt, Hintergrund, Blickwinkel, Lichtstimmung,
...
3. Herunterladen, nach `C:\Users\User\Downloads\Social-Media` verschieben (Ordner
...
- Vorlagen, die du **schon veröffentlicht hast** (aktuell 12 in "Social Media"): auf Entwurf
...
## Seitenverhältnis — hier anders als in den übrigen Kategorien

Social-Media-Posts sind **nicht** hochformatige Poster. Nutze passend zum Pin:
- **1:1 quadratisch** für den klassischen Feed-Post
- **4:5 vertikal** für den Feed-Post mit maximaler Höhe
- **9:16 vertikal** nur für Story- und Reel-Formate

Das Format muss zum Pin passen und **in Prosa im Prompt stehen** ("square 1:1 social
media post"). 111 von 113 bestehenden Prompts nennen ein Format; fehlt es, liefern die
Modelle 1:1 und das Textlayout bricht.
...
Social-Media-Posts typisch: Produkttyp, Produktfarbe, Hintergrundfarbe, Markenname,
Headline, Rabattangabe, Call-to-Action, Kontakt- oder Handle-Zeile.
...
- **`{{image_description}}` muss als letzte Zeile stehen**, nach einer Leerzeile, ohne
  ...
  selbst setzen.
- **Seitenverhältnis in Prosa nennen** — siehe eigener Abschnitt oben.
...
sind unter anderem Nike, Dove, Himalaya, Plum). Von solchen Pins nur Bildaufbau, Licht
und Typografie übernehmen. Der Markenname ist immer die Variable `brand_name`.
...
1. Bild nach `backend/public/__tmp_social.webp` kopieren (Strapi serviert `public/` unter
...
2. Im Admin-Tab per `javascript_tool` das JWT holen. **Beide Quellen prüfen:** früher lag
   ...
   (`JSON.parse(...)` nötig). Dann `fetch('/__tmp_social.webp')` → Blob → FormData →
...
Sobald `C:\Users\User\Downloads\Social-Media` **50 Bilder** enthält: keine weiteren Bilder
...
```json
[
  { "file": "01-velora-shampoo.webp", "verdict": "approved" },
  { "file": "02-vynce-earbuds.webp", "verdict": "rejected",
...
- Es laufen mehrere Claude-Instanzen parallel an anderen Kategorien. Halte dich an den
  Temp-Dateinamen `__tmp_social.webp`, sonst überschreibt ihr euch gegenseitig.
